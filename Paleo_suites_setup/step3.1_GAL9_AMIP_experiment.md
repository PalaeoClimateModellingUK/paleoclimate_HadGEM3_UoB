...
### retrieve_ozone: [ERROR] Unable to access link to model data ($DATAM)
#### Error info
**job.error** of task retrieve_ozone:
```
Traceback (most recent call last):
  File "/home/users/zikun.ren.ext/cylc-run/u-dy662/run20/bin/retrieve_ozone_data.py", line 481, in <module>
    main()
  File "/home/users/zikun.ren.ext/cylc-run/u-dy662/run20/bin/retrieve_ozone_data.py", line 467, in main
    _ = link_sourcefiles(year, False, unlink=True)
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/home/users/zikun.ren.ext/cylc-run/u-dy662/run20/bin/retrieve_ozone_data.py", line 272, in link_sourcefiles
    raise OzoneSourceNotFoundError(
OzoneSourceNotFoundError:
[ERROR] Unable to access link to model data ($DATAM)
[FAIL] ants-launch $CYLC_WORKFLOW_RUN_DIR/bin/retrieve_ozone_data.py # return-code=1
2026-05-22T16:48:31Z CRITICAL - failed/ERR
```
#### reason:
After further test, I found this error disappeared, when I retrigerred the task after the recon completed.     
The model data ```$DATAM``` is in fact linked to the ```share/data/History_Data```, which is generated by the task ```recon```.
#### resolution:
Add ```recon[^] => retrieve_ozone``` to the R1/0101T/P1Y in the [scheduling][[grah]] at ozone-redistribution.cylc.
