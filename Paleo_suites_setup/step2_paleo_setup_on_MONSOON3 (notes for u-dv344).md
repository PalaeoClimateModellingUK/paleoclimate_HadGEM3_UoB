...
**resolution**:      
Tentatively, we haven't got the scripts to generate the `eddy_viscosity_3D.nc`. As a tentative resolution, we switch `nn_ahm_ijk_t` at `nemo > namelist > Dynamics options (namdyn) > Lateral diffusion (namdyn_ldf)` to `Constant (0)`.
##### runoff or river (two kinds of coupling scheme)
...
```
 ...
 571       ENDIF
```
Change the `description` slot of `sn_rcv_rnf` from `coupled1d` to `coupled` at `nemo > namelist > surface boundary condition`    
...
- **STEP3 UM side**:    
According to the section **16.2** in the [umdp_C02 of vn13.8](https://code.metoffice.gov.uk/doc/um/vn13.8/papers/umdp_C02.pdf), in the the STASH, the  `RIVER OUTFLOW (ATMOS GRID)` should be added at `um > namelist > Model Output Streams > STASH Requests and Profiles > STASG Requests`
```
...
##### background bottom turbulent kinetic energy (bfr_coef.nc)
...
**resolution**: swich the **`ln_boost`** to **`spatially uniform drag coefficient`** at `nemo > namelist > Top and bottom boundaries > bottom friction (nambfr)`.


#### UM
##### Chlorophyll qrclim.sea (potential element)
This is the same in both the piControl and eocene in the GC3 suite. In GC5, it is updated. Not sure whether it will significantly impact the results.
