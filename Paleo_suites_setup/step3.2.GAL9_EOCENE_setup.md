...
The generation of SST follows the procedures below:
- run the `regrid_interpolate.py`
- use the `xancil` to generate the sst ancillary. Please note that when using xancil to generate ancillary, the `version number` at 'Configuration > General configuration' must be set to a relatively higher version (like 10.7), or the direction of latitude can be from north to south, which will result in the error shown as below:
```
????????????????????????????????????????????????????????????????????????????????
??????????????????????????????      WARNING       ??????????????????????????????
?  Warning code: -15000
...
?          Expected p grid y start : -90.6250000000
?          Ancil    p grid y start : 90.6250000000
?          Ancil file : /home/users/zikun.ren.ext/cylc-run/u-dy316/run6/share/data/etc/ancil/qrclim.sst
?          Lookup num : 12
?          Stashcode  : 24
...
?        -- /home/users/zikun.ren.ext/cylc-run/u-dy316/run6/share/data/etc/ancil/qrclim.sst
?
